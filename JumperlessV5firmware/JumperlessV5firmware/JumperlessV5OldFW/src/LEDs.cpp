...
Adafruit_NeoMatrix matrix = Adafruit_NeoMatrix(30, 5, LED_PIN,
                                               NEO_MATRIX_TOP + NEO_MATRIX_LEFT +
                                                   NEO_MATRIX_COLUMNS + NEO_MATRIX_PROGRESSIVE,
                                               NEO_GRB + NEO_KHZ800);

const uint16_t colors[] = {
    matrix.Color(70, 0, 50), matrix.Color(0, 30, 0), matrix.Color(0, 0, 8)};
 ...
 */
    ...
    for (int i = 8; i < numberOfNets; i++)
    {
        ...
        else
        {
            ...
            hue = (i - 8) * colorDistance;

            // for (uint8_t hueScan = hue + (colorDistance / 16); hueScan <= 254; hueScan += (colorDistance))
            // {
            //     for (int k = 0; k < 8; k++)
            //     {
            ...
            if (net[netNumber].machine == true)
            {
                ...
                if (net[netNumber].nodes[j] == node || node == -1)
                {
                    if (onOff == 1)
                    {
                        ...
                        {
                            leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 0, scaleDownBrightness(net[netNumber].rawColor));
                            leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 1, scaleDownBrightness(net[netNumber].rawColor));
                            leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 2, scaleDownBrightness(net[netNumber].rawColor));
                            leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 3, scaleDownBrightness(net[netNumber].rawColor));
                            leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 4, scaleDownBrightness(net[netNumber].rawColor));
                ...
                if (net[netNumber].nodes[j] <= NANO_A7)
                {
                    if (net[netNumber].nodes[j] == node || node == -1)
                    {
                        if (onOff == 1)
                        {
                            ...
                            if (net[netNumber].nodes[j] >= NANO_D0)
                            {
                                leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) + 320, color);
                            }
                            else
                            {
                                leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 0, color);
                                leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 1, color);
                                leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 2, color);
                                leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 3, color);
                                leds.setPixelColor((nodesToPixelMap[net[netNumber].nodes[j]]) * 5 + 4, color);
                            }
...
void logoSwirl(int start, int spread)
{
    ...
    leds.setPixelColor(438, dimLogoColor(logoColors[(start + (spread * 2)) % 42]));
    leds.setPixelColor(439, dimLogoColor(logoColors[(start + (spread * 3)) % 42]));
    leds.setPixelColor(440, dimLogoColor(logoColors[(start + (spread * 4)) % 42]));
    leds.setPixelColor(441, dimLogoColor(logoColors[(start + (spread * 5)) % 42]));
    leds.setPixelColor(442, dimLogoColor(logoColors[(start + (spread * 6)) % 42]));
    leds.setPixelColor(443, dimLogoColor(logoColors[(start + (spread * 7)) % 42]));
        ...
        if (j == rail || rail == -1)
        {
            ...
            for (int i = 0; i < 25; i++)
            {
                ...
                if (onOff == 1)
                {
                    ...
                    leds.setPixelColor(railsToPixelMap[j][i], scaleDownBrightness(color, 5, 25));
                }
                else
                {
