...
def package_linux():
    ...
    # Create Python folders for Linux distributions
    print("Creating Python development folders for Linux...")
    linux_python_folders = [
        pathlib.Path("JumperlessLinux_x64/Jumperless Python/"),
        pathlib.Path("JumperlessLinux_arm64/Jumperless Python/") if arm64_success else None
    ]
    
    for folder in linux_python_folders:
        if folder is None:
            continue
        ...
        python_bridge_file = folder / "JumperlessWokwiBridge.py"
        python_requirements_file = folder / "requirements.txt"
        python_launcher_file = folder / "jumperless_launcher.sh"
        ...
        # Copy latest files
        shutil.copy2("JumperlessWokwiBridge.py", folder)
        shutil.copy2("requirements.txt", folder)
        shutil.copy2("jumperless_launcher.sh", folder)
        os.chmod(folder / "jumperless_launcher.sh", 0o755)
        
        print(f"Updated Python files and Linux launcher in {folder}")
