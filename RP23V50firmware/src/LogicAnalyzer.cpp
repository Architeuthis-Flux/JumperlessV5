...
bool calculateAndAllocateBuffers() {
  ...
  // For memory allocation, use the physical sample count (what we actually
  // store) For actual capture, use the effective sample count (what the driver
  // will receive)
  ...
  // Cap the requested sample count to what we can actually capture (physical
  // memory limit) We need to cap based on physical memory, not effective
  // samples
  uint32_t memory_physical_max= req.max_samples_final;
  uint32_t physical_requested= sample_count / calculateOversamplingFactor();

  DEBUG_LA_PRINTF("◆ CAPPING CHECK: Requested %lu effective samples (%lu "
                  "physical) vs memory limit %lu\n",
                  sample_count, physical_requested, memory_physical_max);

  if (physical_requested > memory_physical_max) {
    DEBUG_LA_PRINTF("◆ CAPPING: Physical requested %lu > memory limit %lu, "
                    "capping to memory limit\n",
    ...
    DEBUG_LA_PRINTF("◆ NO CAPPING: Physical requested %lu <= memory limit %lu, "
                    "using requested %lu effective\n",
  ...
  uint32_t digital_samples_to_store=
      samples_to_capture;                                                               // Digital gets full oversampled data
  ...
  if (digital_samples_to_store >
      memory_physical_max * calculateOversamplingFactor()) {
    DEBUG_LA_PRINTF(
        "◆ SAFETY CAPPING: Digital samples %lu > memory limit %lu, capping\n",
        digital_samples_to_store,
        memory_physical_max * calculateOversamplingFactor());
    digital_samples_to_store=
        memory_physical_max * calculateOversamplingFactor();
  ...
  if (analog_samples_to_store > memory_physical_max) {
    DEBUG_LA_PRINTF(
        "◆ SAFETY CAPPING: Analog samples %lu > memory limit %lu, capping\n",
        analog_samples_to_store, memory_physical_max);
    analog_samples_to_store= memory_physical_max;
  }

  size_t actual_digital_bytes=
      digital_samples_to_store * req.digital_storage_per_sample;
  size_t actual_analog_bytes=
      analog_samples_to_store * req.analog_storage_per_sample;
  size_t total_needed= actual_digital_bytes + actual_analog_bytes;

  DEBUG_LA_PRINTF(
      "◆ ALLOCATION CALC: Digital %lu samples × %lu bytes = %lu bytes\n",
      digital_samples_to_store, req.digital_storage_per_sample,
      actual_digital_bytes);
  DEBUG_LA_PRINTF(
      "◆ ALLOCATION CALC: Analog %lu samples × %lu bytes = %lu bytes\n",
      analog_samples_to_store, req.analog_storage_per_sample,
      actual_analog_bytes);
  DEBUG_LA_PRINTF("◆ ALLOCATION CALC: Total needed = %lu + %lu = %lu bytes\n",
                  actual_digital_bytes, actual_analog_bytes, total_needed);

  // Final safety check: if total needed exceeds available memory, reduce both
  // buffers proportionally
  if (total_needed > req.usable_memory) {
    DEBUG_LA_PRINTF("◆ FINAL SAFETY: Total %lu bytes > available %lu bytes, "
                    "reducing buffers\n",
                    total_needed, req.usable_memory);

    // Calculate reduction factor
    float reduction_factor= (float)req.usable_memory / total_needed;
    uint32_t new_digital_samples=
        (uint32_t)(digital_samples_to_store * reduction_factor);
    uint32_t new_analog_samples=
        (uint32_t)(analog_samples_to_store * reduction_factor);

    // Ensure we don't go below minimum
    if (new_digital_samples < JL_LA_MIN_SAMPLES)
      new_digital_samples= JL_LA_MIN_SAMPLES;
    if (new_analog_samples < JL_LA_MIN_SAMPLES)
      new_analog_samples= JL_LA_MIN_SAMPLES;
    ...
    DEBUG_LA_PRINTF("◆ FINAL SAFETY RESULT: Digital %lu samples, Analog %lu "
                    "samples, Total %lu bytes\n",
                    digital_samples_to_store, analog_samples_to_store,
                    total_needed);
  }
  ...
  if (total_needed > (current_free - req.safety_reserve)) {
    DEBUG_LA_PRINTF("◆ ERROR: Cannot allocate %lu bytes (need %lu, have %lu, "
                    "reserve %lu)\n",
                    total_needed, total_needed, current_free,
                    req.safety_reserve);
    return false;
  }

  // Conservative check: ensure we can allocate the larger buffer after the
  // smaller one This accounts for memory fragmentation issues
  size_t available_after_digital=
      current_free - req.safety_reserve - actual_digital_bytes;
  size_t fragmentation_safety_margin=
      1024; // 1KB safety margin for fragmentation
  if (actual_analog_bytes >
      (available_after_digital - fragmentation_safety_margin)) {
    DEBUG_LA_PRINTF("◆ CONSERVATIVE CHECK: Analog buffer %lu bytes > available "
                    "after digital %lu bytes (with %lu safety margin)\n",
                    actual_analog_bytes, available_after_digital,
                    fragmentation_safety_margin);

    // Reduce both buffers to fit within available memory
    float reduction_factor= (float)(current_free - req.safety_reserve -
                                    fragmentation_safety_margin) /
                            total_needed;
    uint32_t new_digital_samples=
        (uint32_t)(digital_samples_to_store * reduction_factor *
                   0.90); // 10% extra safety
    uint32_t new_analog_samples=
        (uint32_t)(analog_samples_to_store * reduction_factor * 0.90);
    ...
    DEBUG_LA_PRINTF("◆ CONSERVATIVE REDUCTION: Digital %lu samples, Analog %lu "
                    "samples, Total %lu bytes\n",
                    digital_samples_to_store, analog_samples_to_store,
                    total_needed);

    // Additional verification: ensure the reduced sizes will actually fit
    size_t available_after_digital_reduced=
        current_free - req.safety_reserve - actual_digital_bytes;
    if (actual_analog_bytes >
        (available_after_digital_reduced - fragmentation_safety_margin)) {
      DEBUG_LA_PRINTF("◆ WARNING: Reduced analog buffer still too large, "
                      "applying additional reduction\n");
      // Apply additional reduction
      float additional_reduction= (float)(available_after_digital_reduced -
                                          fragmentation_safety_margin) /
                                  actual_analog_bytes * 0.95;
      new_analog_samples=
          (uint32_t)(analog_samples_to_store * additional_reduction);
      if (new_analog_samples < JL_LA_MIN_SAMPLES)
        new_analog_samples= JL_LA_MIN_SAMPLES;
      ...
      DEBUG_LA_PRINTF("◆ ADDITIONAL REDUCTION: Digital %lu samples, Analog %lu "
                      "samples, Total %lu bytes\n",
                      digital_samples_to_store, analog_samples_to_store,
                      total_needed);
    }
  ...
  if (actual_digital_bytes > 0) {
    ...
    DEBUG_LA2_PRINTF("◆ Digital buffer allocated: %lu bytes\n",
                     actual_digital_bytes);
  }
  ...
  if (actual_analog_bytes > 0) {
    ...
    DEBUG_LA2_PRINTF("◆ Analog buffer allocated: %lu bytes\n",
                     actual_analog_bytes);
  }

  DEBUG_LA2_PRINTF(
      "◆ Buffer allocation successful. Digital: %lu effective samples, Analog: "
      "%lu physical samples, Digital: %lu B, Analog: %lu B\n",
      digital_samples_to_store, analog_samples_to_store, actual_digital_bytes,
      actual_analog_bytes);
...
void invalidateBufferCache() {
  DEBUG_LA2_PRINTF(
      "◆ CACHE INVALIDATED: Next calculation will be from scratch\n");
...
bool allocateLogicAnalyzerResources() {
  DEBUG_LA_PRINTF(
      "◆ Allocating logic analyzer resources using atomic system...\n");
  ...
  DEBUG_LA2_PRINTLN("◆ PIO/DMA reset complete");
  ...
  DEBUG_LA2_PRINTLN("◆ Attempting PIO allocation...");
  ...
  for (int i= 0; i < 3 && !pio_allocated; i++) {
    ...
    debug_pio_program();  // Debug: Show PIO program
    la_sm= sm;

    DEBUG_LA_PRINTF("◆ Setting up PIO interrupt: PIO%d SM%d, program offset=%d\n", 
                    pio_get_index(la_pio), sm, la_prog_offset);
    
    // Get the interrupt number for this PIO state machine
    int irq_num = pio_interrupt_get(la_pio, sm);
    DEBUG_LA_PRINTF("◆ PIO interrupt number: %d\n", irq_num);
    
    // Use shared handler for PIO1_IRQ_0 (system-level interrupt)
    irq_add_shared_handler(PIO1_IRQ_0, la_counter_handler, 
                          PICO_SHARED_IRQ_HANDLER_DEFAULT_ORDER_PRIORITY);
    irq_set_enabled(PIO1_IRQ_0, true);
    
    // Enable PIO interrupt 0 at the PIO level
    pio_set_irq0_source_enabled(la_pio, pis_interrupt0, true);
    DEBUG_LA_PRINTF("◆ PIO interrupt 0 enabled for PIO%d\n", pio_get_index(la_pio));

    // Test: Check if interrupt is enabled
    // bool irq1_enabled = pio_irq_get(la_pio, 1);
    // DEBUG_LA_PRINTF("◆ PIO interrupt 1 status: %s\n", irq1_enabled ? "SET" : "CLEAR");

    pio_allocated= true;
    DEBUG_LA2_PRINTF("◆ SUCCESS: PIO%d SM%d offset=%d\n", pio_get_index(la_pio),
                     sm, la_prog_offset);
  }

  if (!pio_allocated) {
    DEBUG_LA_PRINTLN(
        "◆ ERROR: Failed to allocate PIO resources after trying all instances");
    releaseLogicAnalyzerResources();
    return false;
  }
  ...
  if (!la_buffer) {
    DEBUG_LA_PRINTF(
        "◆ ERROR: Digital buffer not allocated by atomic function\n");
    releaseLogicAnalyzerResources();
    return false;
  }
  ...
  const char* mode_str=
      (current_la_mode == LA_MODE_DIGITAL_ONLY)   ? "digital-only"
      : (current_la_mode == LA_MODE_MIXED_SIGNAL) ? "mixed-signal"
                                                  : "analog-only";
...
void releaseLogicAnalyzerResources() {
  // Clean up interrupt handler first
...
void handleLogicAnalyzer() {
  ...
  uint32_t current_time= millis();
  if (current_time - last_handler_call <
      5) { // Limit to max 200Hz for better stability
    return;
  }
...
void printLogicAnalyzerMemoryInfo() {
  ...
  if (current_la_mode == LA_MODE_MIXED_SIGNAL) {
    Serial.printf("  Analog Channels: %d enabled (mask=0x%08X)\n",
                  analog_chan_count, analog_mask);
    Serial.printf("  Storage Efficiency: %d bytes/sample (capture) vs %d "
                  "bytes/sample (transmission)\n",
                  storage_bytes, transmission_bytes);

    // Calculate memory savings
    uint32_t old_bytes_per_sample= 32; // Old fixed size
    uint32_t memory_saved_per_sample=
        old_bytes_per_sample - transmission_bytes;
    ...
    if (memory_saved_per_sample > 0) {
      ...
      Serial.printf("  Sample Count Improvement: %lu samples vs %lu samples "
                    "(old method)\n",
                    jl_la_max_samples,
                    (jl_la_buffer_size / old_bytes_per_sample));
    }
  }
}
...
bool initAnalogSampleTimer(uint32_t sample_rate) {
  ...
  if (analog_sample_interval_us > 100000)
    analog_sample_interval_us= 100000; // Cap at 10 Hz minimum
  if (analog_sample_interval_us < 1)
    analog_sample_interval_us= 1; // Cap at 1 MHz maximum
  ...
  DEBUG_LA_PRINTLN(
      "◆ WARNING: Default alarm pool is disabled in build configuration");
  return false;
#else
  DEBUG_LA_PRINTLN(
      "◆ INFO: Default alarm pool is enabled in build configuration");
#endif
#else
  DEBUG_LA_PRINTLN("◆ INFO: Default alarm pool setting not explicitly defined "
                   "(should be enabled)");
#endif
  ...
  if (default_pool == NULL) {
    DEBUG_LA_PRINTLN(
        "◆ ERROR: Default alarm pool is NULL - hardware timers not available");
    return false;
  }

  DEBUG_LA_PRINTF("◆ Hardware timer initialized: %lu μs intervals (%lu Hz), "
                  "alarm pool @ %p\n",
                  analog_sample_interval_us,
                  1000000UL / analog_sample_interval_us, default_pool);
  return true;
}
...
bool startAnalogSampleTimer(uint32_t total_samples) {
  ...
  DEBUG_LA_PRINTF(
      "◆ Creating repeating timer with %ld μs interval (time reference mode)\n",
  ...
  DEBUG_LA_PRINTF(
      "◆ Hardware timer started as time reference (%lu μs intervals)\n",
      analog_sample_interval_us);
...
uint32_t getAnalogTimerTicks() { return analog_timer_ticks; }
...
bool captureAnalogDataDMA() {
  ...
  adc_fifo_setup(
      ...
      1,     // DREQ asserted when at least 8 samples present (better buffering)
  ...
  // conversion time
  uint32_t actual_adc_channels=
      __builtin_popcount(analog_mask & 0xFF); // Only count enabled ADC channels
  ...
  float actual_sample_rate=
      48000000.0f / ((clk_div + 1.0f) * total_cycles_per_round_robin);
...
//     req.digital_storage_per_sample = 1;
//     req.analog_storage_per_sample = (current_la_mode == LA_MODE_DIGITAL_ONLY)
//     ? 0 : req.driver_analog_channels * 2; size_t total_storage_per_sample =
//     req.digital_storage_per_sample + req.analog_storage_per_sample; if
//     (total_storage_per_sample == 0) total_storage_per_sample = 1;

//     // Calculate max samples based on memory
//     req.max_samples_memory_limit = req.usable_memory /
//     total_storage_per_sample;

//     // Adjust for oversampling
//     if (sample_rate > ADC_MAX_SAMPLE_RATE) {
//         float decimation_factor = (float)sample_rate / ADC_MAX_SAMPLE_RATE;
//         req.max_samples_memory_limit =
//         (uint32_t)((float)req.max_samples_memory_limit * decimation_factor);
//     }
...
//     req.digital_buffer_bytes = req.max_samples_final *
//     req.digital_storage_per_sample; req.analog_buffer_bytes =
//     req.max_samples_final * req.analog_storage_per_sample;
//     req.total_buffer_bytes = req.digital_buffer_bytes +
//     req.analog_buffer_bytes;
