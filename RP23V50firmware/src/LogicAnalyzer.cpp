...
#define DEBUG_LA2_PRINTF(fmt, ...) do { if(debugLA2) { Serial.printf(fmt, ##__VA_ARGS__); } } while(0)
#define DEBUG_LA2_PRINTLN(x) do { if(debugLA2) { Serial.println(x); } } while(0)
...
bool la_usb_connected() {
    ...
    // Add debouncing to prevent false disconnection detection
    ...
    while (offset < len) {
        ...
        while (USBSer2.availableForWrite() < chunk_size) {
            ...
            if (wait_time > max_wait_time) {
                DEBUG_LA_PRINTF("◆ WARNING: USB buffer full after %lu ms - continuing\n", wait_time);
                return;
            }
...
bool calculateAndAllocateBuffers() {
    ...
    if (total_needed > req.usable_memory) {
        ...
        DEBUG_LA_PRINTF("◆ FINAL SAFETY RESULT: Digital %lu samples, Analog %lu samples, Total %lu bytes\n", 
    ...
    // Conservative check: ensure we can allocate the larger buffer after the smaller one
    // This accounts for memory fragmentation issues
    size_t available_after_digital = current_free - req.safety_reserve - actual_digital_bytes;
    size_t fragmentation_safety_margin = 1024;  // 1KB safety margin for fragmentation
    if (actual_analog_bytes > (available_after_digital - fragmentation_safety_margin)) {
        DEBUG_LA_PRINTF("◆ CONSERVATIVE CHECK: Analog buffer %lu bytes > available after digital %lu bytes (with %lu safety margin)\n",
                       actual_analog_bytes, available_after_digital, fragmentation_safety_margin);
        
        // Reduce both buffers to fit within available memory
        float reduction_factor = (float)(current_free - req.safety_reserve - fragmentation_safety_margin) / total_needed;
        uint32_t new_digital_samples = (uint32_t)(digital_samples_to_store * reduction_factor * 0.90); // 10% extra safety
        uint32_t new_analog_samples = (uint32_t)(analog_samples_to_store * reduction_factor * 0.90);
        ...
        DEBUG_LA_PRINTF("◆ CONSERVATIVE REDUCTION: Digital %lu samples, Analog %lu samples, Total %lu bytes\n", 
                       digital_samples_to_store, analog_samples_to_store, total_needed);
        
        // Additional verification: ensure the reduced sizes will actually fit
        size_t available_after_digital_reduced = current_free - req.safety_reserve - actual_digital_bytes;
        if (actual_analog_bytes > (available_after_digital_reduced - fragmentation_safety_margin)) {
            DEBUG_LA_PRINTF("◆ WARNING: Reduced analog buffer still too large, applying additional reduction\n");
            // Apply additional reduction
            float additional_reduction = (float)(available_after_digital_reduced - fragmentation_safety_margin) / actual_analog_bytes * 0.95;
            new_analog_samples = (uint32_t)(analog_samples_to_store * additional_reduction);
            if (new_analog_samples < JL_LA_MIN_SAMPLES) new_analog_samples = JL_LA_MIN_SAMPLES;
            ...
            DEBUG_LA_PRINTF("◆ ADDITIONAL REDUCTION: Digital %lu samples, Analog %lu samples, Total %lu bytes\n", 
                           digital_samples_to_store, analog_samples_to_store, total_needed);
        }
    }
    ...
    DEBUG_LA2_PRINTF("◆ Buffer allocation successful. Digital: %lu effective samples, Analog: %lu physical samples, Digital: %lu B, Analog: %lu B\n",
...
void invalidateBufferCache() {
    DEBUG_LA2_PRINTF("◆ CACHE INVALIDATED: Next calculation will be from scratch\n");
...
bool allocateLogicAnalyzerResources() {
    ...
    PIO pio_instances[] = { pio2, pio1, pio0 };
...
void handleConnectionStateChange(bool connected) {
    ...
    if (current_time - last_connection_change < 100) {  // 100ms minimum between changes
...
void sendJumperlessHeader() {
    ...
    // Use actual allocated buffer sizes for max_memory_depth instead of theoretical maximum
    // This prevents the UI from showing incorrect sample limits after memory fragmentation
    ...
    if (jl_la_max_samples > 0) {
        ...
        DEBUG_LA_PRINTF("◆ HEADER: Using theoretical max samples: %lu (no buffers allocated)\n", actual_max_samples);
    }
...
void sendDigitalData() {
    ...
    const uint32_t CHUNK_SIZE = 48;  // 16 samples * 3 bytes - smaller chunks for better flow control
...
void sendMixedSignalData() {
    ...
    const uint32_t CHUNK_SIZE = 256; // Smaller chunks for better flow control - 8 samples per chunk

    // Use static buffer to avoid malloc/free during critical transmission
    static uint8_t chunk_buffer[256];
    uint32_t chunk_pos = 0;
    ...
    if (capture_used_oversampling) {
        ...
        DEBUG_LA_PRINTF("◆ Sending unified mixed-signal data: %lu samples, 32 bytes/sample format\n", sample_count);
    }
    ...
    if (expected_analog_channels != analog_chan_count) {
        DEBUG_LA_PRINTF("◆ WARNING: Channel count mismatch: expected %lu from mask, got %d from calculation\n",
    ...
    for (uint32_t sample = 0; sample < sample_count; sample++) {
        ...
        for (int channel = 0; channel < 14; channel++) {
            ...
            if (channel < 8 && (analog_mask & (1UL << channel))) {
                ...
                // Search through the captured samples to find one that matches this channel
                ...
                for (uint32_t adc_sample_offset = 0; adc_sample_offset < analog_chan_count && !found_channel_sample; adc_sample_offset++) {
                    uint32_t buffer_index = buffer_sample * analog_chan_count + adc_sample_offset;
                    
                    // Check bounds
                    size_t max_buffer_elements = analog_buffer_size / sizeof(uint16_t);
                    if (buffer_index >= max_buffer_elements) break;
                    
                    // Extract embedded channel info from upper 4 bits of sample
                    uint16_t sample_value = analog_buffer[buffer_index];
                    uint8_t embedded_channel = (sample_value >> 12) & 0x0F;  // Extract channel from bits [15:12]
                    uint16_t adc_result = sample_value & 0x0FFF;  // Extract 12-bit ADC result from bits [11:0]
                    ...
                    if (embedded_channel == channel) {
                        ...
                        if (sample < 5) {
                            DEBUG_LA_PRINTF("◆ EMBEDDED Sample %lu Ch %d: buffer[%lu] = %d (embedded_ch=%d)\n", 
                                         sample, channel, buffer_index, adc_value, embedded_channel);
                        }
                    }
                }
                ...
                if (!found_channel_sample && sample < 5) {
                    DEBUG_LA_PRINTF("◆ No embedded channel data for Sample %lu Ch %d, using default\n", sample, channel);
                }
    ...
    DEBUG_LA2_PRINTF("◆ Unified mixed-signal data sent: 32 bytes/sample format\n");
}
...
void __not_in_flash_func(embedAinselInSamples)() {
    ...
    // This function will read each ADC sample and embed the expected channel number
    // based on round-robin position
    
    if (!analog_buffer || !adc_dma_complete) return;
    
    // Calculate total transfers completed
    uint32_t total_samples = sample_count;
    uint32_t buffer_samples = capture_used_oversampling ? (total_samples + capture_duplication_factor - 1) / capture_duplication_factor : total_samples;
    uint32_t total_transfers = buffer_samples * analog_chan_count;
    
    DEBUG_LA_PRINTF("◆ Embedding AINSEL channel info in %lu samples (%d channels)\n", total_transfers, analog_chan_count);
    
    // Embed channel info in each sample based on position in round-robin sequence
    for (uint32_t transfer_idx = 0; transfer_idx < total_transfers; transfer_idx++) {
        ...
        uint16_t channel_embedded = (actual_channel << 12) | adc_result;  // Embed channel in bits [15:12]
        ...
        if (transfer_idx < 10) {
            DEBUG_LA_PRINTF("◆ Embedded[%lu]: raw=%d → ch=%d, result=%d, combined=0x%04X\n", 
                         transfer_idx, adc_value, actual_channel, adc_result, channel_embedded);
        }
    }
}
...
bool captureAnalogDataDMA() {
    ...
    // Use physical sample count (not effective count) since analog doesn't get oversampled
          uint32_t physical_samples = sample_count / calculateOversamplingFactor();
      uint32_t actual_adc_samples = adc_oversampling_mode ? (physical_samples + adc_duplication_factor - 1) / adc_duplication_factor : physical_samples;
    ...
    if (adc_oversampling_mode) {
        ...
        DEBUG_LA_PRINTF("◆ DMA transfer calculation: %lu physical samples × %d channels = %lu transfers (%lu bytes)\n",
                     actual_adc_samples, analog_chan_count, total_transfers, dma_buffer_size);
    }
    ...
    // Note: No separate AINSEL buffer needed - channel info will be embedded in samples post-capture
   ...
   // startAnalogSampleTimer(actual_adc_samples);
    ...
    // adc_fifo_drain();
    // DEBUG_LA_PRINTF("◆ ADC FIFO cleared (level=%d)\n", adc_fifo_get_level());
    ...
    uint32_t round_robin_mask = analog_mask & 0xFF;  // All 8 channels (0-7)
    DEBUG_LA_PRINTF("◆ ADC CAPTURE: analog_mask=0x%08X, round_robin_mask=0x%02X\n", analog_mask, round_robin_mask);
    
    // CRITICAL FIX: Ensure we have a valid round-robin mask
    if (round_robin_mask == 0) {
        DEBUG_LA_PRINTLN("◆ WARNING: Zero round-robin mask, using default (channels 0-4)");
    ...
    // Step 3: Force round-robin counter reset by cycling through single channel mode
    DEBUG_LA_PRINTF("◆ Resetting round-robin counter via single channel selection\n");
    ...
    DEBUG_LA_PRINTF("◆ Setting round-robin mask: 0x%02X (starting from channel %d)\n", round_robin_mask, first_channel);





    DEBUG_LA_PRINTF("◆ Round-robin mask set: 0x%02X (channels: ", round_robin_mask);
    for (int i = 0; i < 8; i++) {
        if (round_robin_mask & (1 << i)) {
            DEBUG_LA_PRINTF("%d ", i);
        }
...
// int i = 0;
   ...
   // DEBUG_LA_PRINTF("◆ Waited %d cycles for round-robin to settle\n", i);
    ...
    // Step 9: Let a few samples settle to ensure round-robin is properly synchronized
   // DEBUG_LA_PRINTF("◆ Allowing initial samples to settle\n");
   // delayMicroseconds(500);  // Let 5-10 samples pass to ensure proper synchronization
   ...
   // USBSer2.flush();
    ...
    DEBUG_LA_PRINTF("◆ RAW DMA BUFFER ANALYSIS (first 80 values = 16 samples × %d channels):\n", analog_chan_count  );
      ...
      DEBUG_LA_PRINTF("◆ RAW DMA BUFFER ANALYSIS (first 80 values = 16 samples × 5 channels):\n");
...
void cleanupAdcDma() {
    ...
    // Note: No AINSEL timer or buffer to clean up - channel info is embedded in samples
