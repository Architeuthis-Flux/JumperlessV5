...
static inline void set_micros_per_byte( cdc_line_coding_t const* line_coding ) {
    ...
    usPerByteSerial1 = ( 1000000 / line_coding->bit_rate + 1 ) * ( line_coding->data_bits + line_coding->parity + line_coding->stop_bits );
	serial1baud = line_coding->bit_rate;
	// Serial.println("serial1baud: " + String(serial1baud));
	// Serial.println("uPerByteSerial1: " + String(usPerByteSerial1));
	// Serial.flush();
}
...
static inline void bridge_uart_to_usb( uint8_t itf ) {
    ...
    while ( Serial1.available( ) > 0 ) {
        ...
        if ( Serial1.overflow( ) ) {
            // Serial.println("Serial1 overflow");
            // Serial.flush();
            break;
        }

        if ( Serial1.getBreakReceived( ) ) {
            // Serial.println("Serial1 break received");
            // Serial.flush();
            break;
        }
        ...
        buf[ idx++ ] = (uint8_t)c;
        // Serial.print((char)c);
		// Serial.println(microsPerByteSerial1 * 2);
		// Serial.flush();
		if (Serial1.available() <= 0) {
        delayMicroseconds( usPerByteSerial1 * 3 );
		}
    }
    if ( idx > 0 ) {
