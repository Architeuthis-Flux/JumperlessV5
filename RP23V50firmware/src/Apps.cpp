...
void mp_hal_stdout_tx_strn_cooked(const char *str, size_t len) {
    // Write each character to Arduino Serial, converting \n to \r\n
...
///@brief 0 = menu, 1 = prompt, 2 = output, 3 = input, 4 = error, 5 = prompt alt 1, 6 = prompt alt 2, 7 = prompt alt 3
int replColors[8] = {
    38, //menu
     69, //prompt
      155, //output
...
//   }
 ...
 // Serial.print("Enter any character to start REPL");
  Serial.println();
  Serial.flush();
  ...
  while (true) {
    ...
    if (Serial.available() > 0) {
      ...
      if (c == '\r' || c == '\n') {
        ...
        bool isConsecutiveNewline = (lastChar == '\r' || lastChar == '\n') && 
                                   (currentTime - lastNewlineTime < NEWLINE_DEBOUNCE_MS);
        
        // Only process newline if it's not a consecutive duplicate or if there's actual content
        ...
        if (currentLine == "mem") {
          changeTerminalColor(replColors[0],true);
          mp_embed_exec_str("import gc; print('Free:', gc.mem_free(), 'Allocated:', gc.mem_alloc())");
          changeTerminalColor(replColors[1],true);
          Serial.print(">>> ");
          Serial.flush();
          currentLine = "";
      ...
      } else if (c == ' ' && currentLine.startsWith(">>>") && currentLine.length() == 4) {
        currentLine.remove(0,4);
        Serial.print("\b\b\b\b");
        Serial.flush();
        continue;
    //   } else if (c == '>' && currentLine.startsWith(">>") && currentLine.length() == 3) {
